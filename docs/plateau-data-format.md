...
このプロジェクトでは、道路面と標高の基準をPLATEAUへ統一しています。OSMはバス経路、
停留所、車線方向、一方通行、信号、交差点、道路接続に加えて、PLATEAU道路面へ重ねる
道路中心線・車線境界線・歩道形状の元データとして使用します。
...
| 道路面 | PLATEAU tran | PLATEAU交通ポリゴンを表示 |
| 道路中心線・車線境界線 | OSM topology | OSMの車線構成を使い、PLATEAU道路面の標高へ表示オーバーレイを配置 |
| 歩道 | PLATEAU tran | PLATEAU交通ポリゴンの歩道面を表示 |
| 道路の論理形状 | OSM route/network | PLATEAU地形へ投影して走行ロジックへ利用 |
...
実行時の経路標高はこのグリッドの三角形分割を直接サンプルします。`route-elevation.json`
は生成・検証用の補助成果物であり、実行時の第二の高さ情報には使用しません。
