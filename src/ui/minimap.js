...
export function createMinimap(path, stops) {
  ...
  const routePts = [];
  for (let i = 0; i < path.points.length; i += 8) routePts.push(toXY(path.points[i]));
  ...
  return {
    update(pos, nextStopIndex, dt = 0.1) {
      ...
      // 経路
      ctx.beginPath();
      routePts.forEach(([x, y], i) => (i === 0 ? ctx.moveTo(x, y) : ctx.lineTo(x, y)));
      ...
      stopPts.forEach(([x, y], i) => {
        ...
        else if (i === nextStopIndex) ctx.fillStyle = blink % 0.9 < 0.55 ? '#ffb43c' : '#8a6a30';
