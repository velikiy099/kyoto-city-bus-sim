...
export function createScoring(state) {
  ...
  function add(pts, label, kind = null, toast = true) {
    ...
    if (toast) showToast(`${label} ${pts > 0 ? '+' : ''}${pts}`, kind ?? (pts >= 0 ? 'good' : 'bad'));
  }
  ...
  return {
    ...
    tick(dt, bus, limitMs) {
      ...
      if (Math.abs(bus.latAccel) > S.harshLatThreshold && harshTurnCooldown === 0) {
      ...
      const crossing = state.lateral + CFG.bus.width / 2 > 0.15 && !state.offRoute;
      if (crossing) state.score += Math.round(S.centerlinePerSec * dt * 10) / 10;
      if (crossing && !centerlineActive) showToast('センターラインはみ出し!', 'bad');
