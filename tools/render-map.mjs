...
const data = JSON.parse(readFileSync(join(ROOT, 'src', 'data', 'route18.json'), 'utf8'));
...
function pointAt(s) {
  ...
  while (lo < hi - 1) { const m = (lo + hi) >> 1; (cumLen[m] <= ss ? lo = m : hi = m); }
  const a = path[lo], b = path[Math.min(path.length - 1, lo + 1)];
  const len = cumLen[lo + 1] - cumLen[lo] || 1e-9;
  const t = (ss - cumLen[lo]) / len;
  return [a[0] + (b[0] - a[0]) * t, a[1] + (b[1] - a[1]) * t];
}
function headingAt(s) {
  const a = pointAt(s - 1.5), b = pointAt(s + 1.5);
...
function projectPoint(pt) {
  ...
  for (let i = 0; i < path.length - 1; i++) {
    ...
    const dx = pt[0] - (a[0] + abx * t), dz = pt[1] - (a[1] + abz * t);
    const d2 = dx * dx + dz * dz;
    if (d2 < bd) { bd = d2; bs = cumLen[i] + Math.sqrt(ab2) * t; }
  }
...
function inArm(pt, arm, margin = 0) {
  const dx = pt[0] - arm.cx, dz = pt[1] - arm.cz;
  const dir = [Math.sin(arm.heading), Math.cos(arm.heading)];
  const along = dx * dir[0] + dz * dir[1];
  const lat = dx * dir[1] - dz * dir[0];
  return along > arm.from - margin && along < arm.to + margin && Math.abs(lat) < arm.hw + margin;
}
...
if (CHECK) {
  ...
  for (const sig of data.signals ?? []) {
    ...
    for (const h of sig.heads) {
      ...
      for (const arm of arms) {
        if (inArm(p, arm, -0.3)) {
          report('FAIL', `信号柱が右左折交差点の路面上: s=${sig.s} ${h.kind} pole=(${p[0]}, ${p[1]})`);
        }
      }
      for (const ix of data.intersections ?? []) {
        ...
        const along = dx * dir[0] + dz * dir[1];
        const lat = dx * dir[1] - dz * dir[0];
        if (Math.abs(along) < ix.length / 2 - 0.3 && Math.abs(lat) < ix.width / 2 - 0.3) {
          report('WARN', `信号柱が交差道路スタブ上: s=${sig.s} ${h.kind} pole=(${p[0]}, ${p[1]}) ${ix.name || ''}`);
        }
      }
    }
  }
  ...
  for (const r of data.railStructures ?? []) {
    ...
    const deg = (Math.min(a, Math.PI - a) === a ? a : Math.PI - a) * 180 / Math.PI;
    const cross = 90 - Math.abs(90 - (a * 180) / Math.PI); // 交差角(0..90)
    if (cross < 70) report('FAIL', `線路の交差角が浅い: ${r.name} s=${r.s} 交差角${cross.toFixed(1)}°`);
    else console.log(`PASS  線路交差角 ${r.name} s=${r.s}: ${cross.toFixed(1)}°`);
  }
  ...
  for (const t of data.turnIntersections ?? []) {
    for (const st of data.stops ?? []) {
      if (st.s > t.sIn - 5 && st.s < t.sOut + 5) {
        report('WARN', `停留所「${st.name}」(s=${st.s}) が右左折交差点の円弧内 [${t.sIn}, ${t.sOut}]`);
      }
    }
    if (!t.crossName) report('WARN', `右左折交差点 s=${t.s} (${t.angleDeg}°) に交差道路名がない`);
  }
...
for (let i = 0; i < path.length; i++) {
  ...
  minZ = Math.min(minZ, path[i][1]); maxZ = Math.max(maxZ, path[i][1]);
}
const M = 90; // 余白 [m]
...
const poly = (pts, fill, opacity = 1) =>
  el.push(`<polygon points="${pts.map((p) => `${p[0].toFixed(1)},${p[1].toFixed(1)}`).join(' ')}" fill="${fill}" fill-opacity="${opacity}"/>`);
const line = (a, b, stroke, w, opts = '') =>
  el.push(`<line x1="${a[0].toFixed(1)}" y1="${a[1].toFixed(1)}" x2="${b[0].toFixed(1)}" y2="${b[1].toFixed(1)}" stroke="${stroke}" stroke-width="${w}" ${opts}/>`);
const circle = (p, r, fill) => el.push(`<circle cx="${p[0].toFixed(1)}" cy="${p[1].toFixed(1)}" r="${r}" fill="${fill}"/>`);
const text = (p, str, size, fill = '#333', anchor = 'start') =>
  el.push(`<text x="${p[0].toFixed(1)}" y="${p[1].toFixed(1)}" font-size="${size}" fill="${fill}" text-anchor="${anchor}" font-family="sans-serif">${esc(str)}</text>`);
...
for (const ix of data.intersections ?? []) {
  ...
  const c = pointAt(ix.s);
  poly(rectPts(c[0], c[1], ix.heading, -ix.length / 2, ix.length / 2, ix.width / 2), '#565b61');
}
...
for (const r of data.railStructures ?? []) {
  ...
  poly(rectPts(c[0], c[1], r.heading, -r.length / 2, r.length / 2, (r.width ?? 20) / 2), color, 0.15);
  for (let i = 0; i < n; i++) {
    const off = (i - (n - 1) / 2) * spacing;
    const a = [c[0] + v[0] * off - u[0] * r.length / 2, c[1] + v[1] * off - u[1] * r.length / 2];
    const b = [c[0] + v[0] * off + u[0] * r.length / 2, c[1] + v[1] * off + u[1] * r.length / 2];
    line(a, b, color, 0.8);
  }
...
for (const sig of data.signals ?? []) {
  ...
  for (const h of sig.heads ?? []) {
    ...
    const back = [h.head[0] - Math.sin(h.face) * 4, h.head[1] - Math.cos(h.face) * 4];
...
for (const t of data.turnIntersections ?? []) {
  ...
  text([t.x + 4, t.z - 4], `${t.crossName || '(無名)'} ${t.angleDeg}° s=${t.s}`, 8, '#c0392b');
}
...
console.log(`OK → ${OUT}  (viewBox: ${vb.map((v) => v.toFixed(0)).join(' ')}, 要素数 ${el.length})`);
